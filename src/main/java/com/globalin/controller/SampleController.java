...
public class SampleController {
   ...
   // 다양한 형식의 데이터를 전송할 수 있다.
   // 주로 많이 사용하는 형태 : 문자열, JSON, XML

   // 문자열을 반환하는 형태
   // Controller는 리턴타입이 String인 메소드 :
   // 리턴한 결과가 jsp 파일 이름이 됭써는데, ResrController는 순수한 데이터를 준다
   // produces 속성은 메소드가 생성하는 media type을 의미한다(mime type)
   ...
   public Map<String, SampleVO> getMap() {
	...
	//헤더 정보를 조작해서 헤더의 상태메세지를 같이 포함해서 전달하는 방법
	@GetMapping(value="/check", params= {"height", "weight"})
	public ResponseEntity<SampleVO> check(Double height, Double weight){
		SampleVO s = new SampleVO(0, ""+height, ""+ weight);
		
		//우리가 리턴해줄 RespinseEntity 객체
		ResponseEntity<SampleVO> result = null;
		
		//키가 180 이하이면 BAD_GATEWAY
		if(height <= 180) {
	...
	//RestController 에서 많이 사용하는 URL 경로에 파라미터를 포함하는 방법
	//?파라미터 = 값: 쿼리스트림
	// /파라미터값/
	//이 요청은 요청 url에 파라미터 값이 포함되어 있다.
	//이 파라미터의 이름은 cat, pd 이다 (값이 cat, pid는 아님)
	//PathVariable에는 기본 자료형 사용 X
	@GetMapping("/product/{cat}/{pid}")
	public String[] getPath(@PathVariable("cat") String cat,
			@PathVariable("pid") Integer pid) {
		//String cat 은 url에 {cat} 자리에 있는 값을 가져온다.
		//Integer pid 는 url에 {pid} 자리에 있는 값을 가져온다.
